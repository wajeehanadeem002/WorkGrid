...
Trusted-file deletion uses `ready -> deleting -> deleted/restored`. The database first records a controlled deleting state, Storage removal happens next, and reconciliation deletes metadata only after confirming that the object is absent. If Storage still contains the object, metadata returns to ready instead of becoming a broken download. Rejected upload cleanup uses `pending/verifying -> discarding -> deleted`; it never restores unverified bytes to ready.

The scheduled `cleanup-attachments` Edge Function is protected by an independent Vault-backed token and is the only component allowed to construct an elevated Supabase client. Service-role-only, fixed-argument RPCs select a 50-row batch and finalize metadata; normal clients cannot provide a path or limit. A database lease and `FOR UPDATE SKIP LOCKED` make overlapping invocations retry-safe. The worker uses the official private Storage API with concurrency five and never deletes from `storage.objects` through SQL. See [storage-cleanup.md](storage-cleanup.md) for provisioning, retries, and monitoring.
